"""
Enhanced Discord Quiz Bot with Wikipedia integration and fuzzy matching
"""
...
class QuizBot(commands.Bot):
    ...
    def load_celebrities(self):
        ...
        try:
            ...
            logger.warning("celebrities.json not found, using empty database")
            return {"celebrities": []}
        except Exception as e:
    ...
    def save_celebrities(self):
        """Save celebrities database to JSON file"""
        try:
    ...
    async def list_celebrities(self, ctx):
        ...
        if not pages:
            await ctx.send("📝 No celebrities found.")
            return
        ...
        if len(pages) > 1:
            ...
            while True:
                try:
                    ...
                    embed = self.create_celebrity_list_embed(pages[current_page], current_page + 1, len(pages))
    ...
    def create_celebrity_list_embed(self, celebrities, page, total_pages):
        """Create embed for celebrity list"""
        embed = discord.Embed(
            title="🌟 Arab Celebrity Database",
        ...
        for celebrity in celebrities:
            ...
            if aliases:
                value += f"**Aliases:** {', '.join(aliases)}\n"
            if arabic_name:
                value += f"**Arabic:** {arabic_name}\n"
            
            has_image = "✅" if celebrity.get('image_url') else "❌"
            value += f"**Image:** {has_image}"
